...
# 단일 파일(gubiba_30d_full.csv) 또는 청크(gubiba_30d_chunk_*.csv)를
...
# 파일 탐색
$fullFile = Get-ChildItem "$DownloadsDir\gubiba_30d_full.csv" -ErrorAction SilentlyContinue
$chunkFiles = Get-ChildItem "$DownloadsDir\gubiba_30d_chunk_*.csv" -ErrorAction SilentlyContinue | Sort-Object Name

if ($fullFile) {
    Write-Host "[pickup] Found: $($fullFile.Name) ($([math]::Round($fullFile.Length/1024, 1))KB)"
    Copy-Item $fullFile.FullName "$TargetDir\gubiba_30d_full.csv"
    Write-Host "[pickup] Copied to: $TargetDir\gubiba_30d_full.csv"

    # 기본 검증
    $csv = Import-Csv "$TargetDir\gubiba_30d_full.csv"
    ...
    # 병합 파일 저장
    $mergedPath = "$TargetDir\gubiba_30d_merged.csv"
    ...
    Write-Host "[pickup] No files found in $DownloadsDir matching gubiba_30d_*.csv"
    Write-Host "[pickup] Expected: gubiba_30d_full.csv or gubiba_30d_chunk_001.csv ..."
    exit 1
}

Write-Host ""
Write-Host "[done] Files in $TargetDir :"
Get-ChildItem "$TargetDir\gubiba_30d*" | ForEach-Object { Write-Host "  $($_.Name) ($([math]::Round($_.Length/1024, 1))KB)" }
