...
## 2026-08-05 - SOOP 버튜버 원장은 소프트콘 1차 시드 + 공식 계정 보강으로 운용

decision_id: `DL_POLICY_20260805_052`

scope:
- `consulting/runs/vtuber_registry_20260805/`
- 소프트콘 `platform=afreeca` 행의 SOOP 이관
- SOOP 공식 방송국 핸들 및 URL 연결

what_changed:
SOOP 활동 버튜버의 1차 모집단은 소프트콘의 legacy platform code `afreeca` 행을 `soop`으로 매핑해 사용한다. 다만 소프트콘 `cid`는 SOOP 공식 방송국 핸들과 동일하다고 가정하지 않는다. 공식 소속사·공식 그룹·공식 플랫폼에서 확인한 SOOP 계정이 소프트콘 시드에 없거나 `cid`와 다르면, 공식 계정 후보로 별도 보존하고 신원 교차검증 후 병합한다.

why:
소프트콘 2026-07-04 원장에서 SOOP 시드 109개를 확보했지만, AKAIV 공식 사이트의 SOOP 핸들 5개는 소프트콘 `cid`와 일치하지 않았고 이세계아이돌 공식 SOOP 핸들 6개는 시드에서 발견되지 않았다. `cid`로 공식 URL을 조립하면 잘못된 계정 연결이 생기며, 소프트콘만으로 SOOP 모집단이 완결됐다고 간주하면 대표적 활동자를 누락한다.

authority:
operator 2026-08-05의 “SOOP 활동 버튜버는 소프트콘 데이터를 활용” 지시와, 같은 날 공개 공식 출처 교차검증 결과.

boundary:
소프트콘은 계속 SOOP의 우선 시드다. 이 결정은 소프트콘 데이터를 폐기하거나 공개 웹 전수 크롤링으로 대체하라는 뜻이 아니다. 로그인 세션, 비공개 데이터, 외부 연락은 별도 승인 없이는 사용하지 않는다. 공식 후보와 기존 persona의 병합은 이름 유사성만으로 자동 실행하지 않는다.
