...
Arthur may use an operator-approved ephemeral cookie bridge only as same-origin session delegation.
...
- pass cookie values in memory only to same-origin `curl_cffi`
- record only cookie names, domains, expiry/session summary, scope, and bridge status

Required:

- `CollectDirective.approved=true` for collect
- exact `approved_scope.allowed_urls`
- exact origin allowlist
- explicit bridge enablement such as `allow_ephemeral_cookie_bridge=true`
- passed intent-alignment gate
- no raw HTML, screenshot, debug-log, or secret-value artifact storage
...
- durable cookie/token value storage
- using the bridge to bypass CAPTCHA solvers, credential prompts, private/account pages, or unexpected security pages

If scope, checkpoint, CAPTCHA, private/account page, or secret-persistence uncertainty appears, stop and preserve the boundary.
